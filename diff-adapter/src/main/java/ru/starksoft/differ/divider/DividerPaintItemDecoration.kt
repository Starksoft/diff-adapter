...
abstract class DividerPaintItemDecoration(context: Context, @ColorRes colorResId: Int, @DimenRes dividerWidthDimenResId: Int) :
	...
	protected abstract fun getExtraPadding(@IntRange(from = 0) childPosition: Int): ExtraPadding
	...
	open fun disableDraw(@IntRange(from = 0) childPosition: Int): Boolean {
		return false
	}

	open fun isTopDivider(@IntRange(from = 0) childPosition: Int): Boolean {
		return false
	}
}
